<template>
    <div>
        <a :href="`/cocktail/${cocktail.id}`" class="hero group transition-[background-size] bg-[length:100%_auto] hover:bg-[length:105%_auto]" :style="{
...
<script setup>
    const {data: cocktails} = await useFetch('/api/drink')
</script>
